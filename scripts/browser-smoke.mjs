...
export function buildBrowserSmokeConfig(env = process.env) {
  return {
    appUrl: normalizeUrl(env.MG_BROWSER_SMOKE_APP_URL || DEFAULT_APP_URL),
    worker: buildSmokeConfig(env),
    chromeExecutablePath: env.PLAYWRIGHT_CHROMIUM_EXECUTABLE || DEFAULT_CHROME_PATH,
    sampleImagePath: env.MG_BROWSER_SMOKE_IMAGE || DEFAULT_SAMPLE_IMAGE_PATH,
    downloadDir: env.MG_BROWSER_SMOKE_DOWNLOAD_DIR || path.resolve('_codex/browser-smoke/downloads'),
  }
}
...
export async function runBrowserSmoke(config) {
  ...
  const checks = []
  const albumTitle = `MG Browser Smoke ${Date.now()}`
